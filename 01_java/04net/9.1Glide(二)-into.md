    ...
    public <T, Z, R> LoadStatus load(Key signature, int width,
      ...
      // 新建EngineJob和EngineRunnable对象，并调用EngineJob的start方法开始执行EngineRunnable
        ...
        engineJob.addCallback(cb);
      	//start方法最后会将 EngineRunnable任务添加到线程池中进行执行
...
class EngineRunnable implements Runnable, Prioritized {
    ...
    public EngineRunnable(EngineRunnableManager manager, 
                          DecodeJob<?, ?, ?> decodeJob, 
                          Priority priority) {
  ...
  // 3.最后调用的是DecodeJob的decodeFromSource();方法
...
##### 10.DecodeJob的decodeFromSource方法
...
class DecodeJob<A, T, Z> {
    ...
    public Resource<Z> decodeFromSource() throws Exception {
      // 1.继续调用decodeSource获取Resource对象，
      //然后进行调用transformEncodeAndTranscode方法进行资源的转换
    ...
    private Resource<T> decodeSource() throws Exception {
        ...
        try {
            long startTime = LogTime.getLogTime();
          // 核心方法 2.调用Fetcher的loadData方法加载资源，Fetcher是实现类HttpUrlFetcher
...
public class HttpUrlFetcher implements DataFetcher<InputStream> {
    ...
    private InputStream loadDataWithRedirects(URL url, int redirects, 
      ...
      // 4.根据相应码处理
        ...
        if (statusCode / 100 == 2) {
          // 5.（主流程，获取网络图片资源成功）调用getStreamForSuccessfulRequest方法获取InputStream
...
- 1.into(ImageView )方法
  - 生成GlideDrawableImageViewTarget对象，作为ImageView的承接封装类
  - 发起请求：
    - 前一个prev请求处理，清理并回收
    - 创建当前图片加载的请求-GenericRequest
    - 通过调用RequestTracker的runRequest方法发起请求
  - 调用GenericRequest的begin方法开始执行请求
  - 接着调用GenericRequest的onSizeReady方法
    - 调用Engine的load方法进行图片请求的加载
