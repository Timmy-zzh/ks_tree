...
//        int res = demo.lengthOfLongestSubstring("abcabcbb");
     ...
     * 3.总结
     * -双指针左右表示区间范围，区间范围内元素要求字符没有重复，
     * -当右侧指针右移扩展时，判断右侧新元素与区间范围内的元素是否重复，重复的话需要移动左侧指针位置
     * --双指针表示区间范围，右侧指针不断往右移动添加新的元素到区间内，新元素在添加前后需要保证区间范围内元素不存在重复
     * --如果出现重复，通过右移左指针保证区间内无重复元素
     */
